...
{
    [[ -f "$PACKAGEJSON_PATH" ]] && 
    [[ "$(cat "$PACKAGEJSON_PATH")" =~ \{\{name\}\}.*\{\{description\}\} ]] && 
    [[ -f "$README_PATH" ]] && 
    [[ "$(cat "$README_PATH")" =~ \{\{action-name\}\}.*\{\{name\}\} ]] &&
    [[ -f "$ACTION_PATH" ]] && 
    [[ "$(cat "$ACTION_PATH")" =~ \{\{action-name\}\}.*\{\{description\}\} ]] && 
    [[ -f "$LOCALPAYLOAD_PATH" ]] && 
    [[ "$(cat "$LOCALPAYLOAD_PATH")" =~ \{\{name\}\} ]]
}
...
function _readActionName
{
    local BREAK="0"
    while [[ "$BREAK" == "0" ]] ; do
        printf "${WHITE}Action Name${NC}: "
        read -r
        if [[ "$REPLY" =~ ^[[:space:]]*([a-zA-Z][a-zA-Z0-9 \_-]*)[[:space:]]*$ ]]; then
...
function _replaceTags
{
    ...
    sed "s/{{action-name}}/$ACTIONNAME/g; s/{{name}}/$NAME/g" "$SOURCE_FILE" > "$SWAP_FILE"
    mv "$SWAP_FILE" "$SOURCE_FILE"

    SOURCE_FILE="$ACTION_PATH"
    SWAP_FILE="$SOURCE_FILE.swap"
    sed "s/{{action-name}}/$ACTIONNAME/g; s/{{description}}/$DESCRIPTION/g" "$SOURCE_FILE" > "$SWAP_FILE"
    mv "$SWAP_FILE" "$SOURCE_FILE"

    SOURCE_FILE="$LOCALPAYLOAD_PATH"
    SWAP_FILE="$SOURCE_FILE.swap"
...
{
    rm -rf .git
    local REPO="git@prethora.github.com:prethora/actions__$NAME"
